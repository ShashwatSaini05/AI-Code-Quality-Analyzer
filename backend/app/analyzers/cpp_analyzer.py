...
class CppAnalyzer(BaseAnalyzer):
    ...
    def parse_functions(self, code: str) -> list[FunctionInfo]:
        ...
        lines = code.split("\n")
        pattern = re.compile(r'^(?:\w+\s+)*(\w+)\s*\(([^)]*)\)\s*(?:const)?\s*\{')
        for i, line in enumerate(lines):
            match = pattern.match(line.strip())
            if match:
                name = match.group(1)
                if name in ('if', 'for', 'while', 'switch', 'catch', 'return'): continue
                params_str = match.group(2).strip()
                params = [p for p in params_str.split(",") if p.strip()] if params_str else []
    ...
    def parse_classes(self, code: str) -> list[ClassInfo]:
        ...
        lines = code.split("\n")
        pattern = re.compile(r'(?:class|struct)\s+(\w+)\s*(?::\s*(?:public|private|protected)\s+(\w+))?\s*\{')
        for i, line in enumerate(lines):
            ...
            if match:
                ...
                end_line = self._find_block_end(lines, i)
                classes.append(ClassInfo(name=name, start_line=i+1, end_line=end_line+1, parent_classes=parents))
        return classes
